...
class MMK2FK:
    def __init__(self, mjcf_path=None):
        if mjcf_path is None:
            task_dir = Path(DISCOVERSE_ROOT_DIR) / "examples" / "supermarket_sorting"
            src = task_dir / "mjcf" / "retail_competition.xml"
            runtime = Path("/tmp/retail_competition_fk.xml")
            runtime.write_text(src.read_text().replace("__REPO_ROOT__", str(task_dir)))
