#!/usr/bin/env bash
# 4 rounds x 5 orders, 30 min per round (forced restart), 5-min log checks.
set -u
LOGDIR=/home/zwt/runner_test_logs/auto4x5_$(date +%Y%m%d_%H%M%S)
mkdir -p "$LOGDIR"
...
for round in 1 2 3 4; do
  ...
  # up to 6 x 5-min checks (30 min), forced restart at end; early exit if client gone
  for i in $(seq 1 6); do
    sleep 300
    ...
    echo zwt | sudo -S docker logs supermarket_sorting_client 2>&1 | grep -E "order id=|PLACE COMPLETE|match finished|delivered|failed|starting order" | tail -6 >>"$LOGDIR/round${round}_progress.log"
    echo zwt | sudo -S docker logs supermarket_sorting_client 2>&1 | grep -E "\[fatal\]|Traceback|\[ERROR\]" | tail -5 >>"$LOGDIR/round${round}_issues.log"
  done

  echo "=== ROUND $round forced restart $(date '+%T') ==="
  echo zwt | sudo -S docker rm -f supermarket_sorting_client supermarket_sorting_server >/dev/null 2>&1
done
echo "=== ALL 4 ROUNDS DONE $(date '+%F %T') ==="
