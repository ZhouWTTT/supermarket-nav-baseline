...
def test_sphere_post_extension_uses_gentle_twenty_mm_per_second_limit():
    ...
    assert np.allclose(policy.post_extend_target_world, [1.0, 2.065, 3.0])
    assert policy.post_extend_speed_mps == 0.020
    assert math.isclose(policy.post_extend_duration_s, 4.875)
    assert policy.state == "post_extend"


def test_generic_post_extension_keeps_existing_speed_and_duration():
    policy = Harness(sphere=False)
    policy.start_post_extension()

    assert policy.post_extend_speed_mps == 0.042
    assert math.isclose(policy.post_extend_duration_s, 2.5)
    assert policy.state == "post_extend"
