...
Set ``SUPERMARKET_NAV_SEQUENCE`` to a comma-separated list such as
``shelf_D,delivery,shelf_D`` to run an exact experiment route.
Otherwise, ``SUPERMARKET_NAV_SHELVES`` controls the shelf list used by the
default delivery/shelf alternating sequence.
"""
...
# ---------------------------------------------------------------------------
def _build_sequence():
    raw_sequence = os.environ.get("SUPERMARKET_NAV_SEQUENCE", "").strip()
    if raw_sequence:
        sequence = []
        for raw_name in raw_sequence.split(","):
            name = raw_name.strip()
            if not name:
                continue
            key = name.lower()
            ...
            if key.startswith("shelf_"):
                shelf = key.split("_", 1)[1].upper()
            else:
                shelf = key.upper()
            if shelf not in SHELF_APPROACH:
                raise ValueError(
                    "SUPERMARKET_NAV_SEQUENCE only accepts delivery or shelf_A-E; "
                    f"invalid: {name}")
            sequence.append((f"shelf_{shelf}", SHELF_APPROACH[shelf]))
        if not sequence:
            raise ValueError("SUPERMARKET_NAV_SEQUENCE is empty after parsing")
        return sequence
...
class NavigationDemoNode(Node):
    ...
    def __init__(self):
        ...
        self.skip_tucking = (
            os.environ.get("SUPERMARKET_NAV_SKIP_TUCKING", "1") == "1")
        self.state = "NAVIGATING" if self.skip_tucking else "TUCKING"
        ...
        self.get_logger().info(
            ...
            f" skip_tucking={int(self.skip_tucking)}")
    ...
    def _next_goal(self):
        ...
        self.get_logger().info(
            f"path_memory_goal={self.navigator.path_memory_status()}")
        self.state = "TUCKING"
        self._arms_tucked = False
    ...
    def _tick_impl(self):
        ...
        if reached:
            ...
            self.get_logger().info(
                ...
                f"{clearance_text}")
            self.get_logger().info(
                f"path_memory_reached={self.navigator.path_memory_status()}")
            self.state = "DWELLING"
