#!/usr/bin/env python3
"""超市分拣 - competition_runner 一键启动 GUI。

一键启动官方仿真 Server 与正式比赛入口 ``competition_runner.py``：
...
RUNTIME_DIR_CONTAINER = (
    "/workspace/supermarket_sorting_task/logs/competition_runner")
...
class LauncherApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("超市分拣 - competition_runner 启动器")
    ...
    def _build_controls(self):
        ...
        self.match_timeout_var = tk.IntVar(value=1800)
        ...
        self.skip_recheck_var = tk.BooleanVar(value=True)
        ...
        ttk.Checkbutton(
            row4, text="跳过抓前核验(close-recheck)",
        ...
        ttk.Label(
            ...
            text="提示：任务列表默认随机 5 个固定槽位(product_XXX)，也可手写"
                 ...
                 "logs/competition_runner/<run_prefix>/。",
    ...
    def _build_memory_matrix_tab(self, tabs):
        """3 层 × 5 货架 × 3 列的记忆矩阵可视化面板（复用 snapshot GUI 版）。"""
    ...
    def _runner_args(self) -> list[str]:
        ...
        if not self.dynamic_direct_var.get():
            runner_flags.append("--no-dynamic-direct")
        if not self.skip_recheck_var.get():
            runner_flags.append("--close-recheck")
