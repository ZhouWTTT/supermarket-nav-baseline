...
class LauncherApp:
    ...
    def _build_controls(self):
        ...
        # 最近优先抓取：默认开启，从记忆矩阵里选择离机器人最近的待办货物；
        # 取消后严格按订单列表顺序抓取。
        self.nearest_var = tk.BooleanVar(value=True)
        ...
        ttk.Checkbutton(
            row2b, text="最近优先抓取",
            variable=self.nearest_var).pack(side="left", padx=(0, 8))
    ...
    def _client_args(self) -> list[str]:
        ...
        # 当前客户端默认权重指向仓库根 best.pt（不存在），必须显式传
        # perception/checkpoints 下的权重文件。
        weights = (
    ...
    def _start_recorder(self):
        if not self.record_var.get():
            ...
            return
        if not os.path.isfile(os.path.join(REPO_ROOT, "record_run.py")):
            self._append_log(
                self.client_text,
                "录像启动失败: 缺少 record_run.py（本次不生成视频）。\n")
            return
